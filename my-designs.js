...
async function renameDesign(designId, newName) {
    ...
    if (designData.order_placed > 0) {
      window.translatedAlert('design_ordered_no_rename');      // Revert the name change in the UI
    ...
    // If not ordered, proceed with renaming
    const { data, error } = await supabase.from("user_files").update({ custom_name: newName }).eq("id", designId)
