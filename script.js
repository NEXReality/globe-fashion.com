...
async function saveDesign() {
    ...
    try {
        ...
        displayFeedback('error_saving_design', true);
    } finally {
        isSaving = false;
...
function displayFeedback(messageKey, isError = false, ...args) {
    ...
    if (feedbackElement) {
        ...
        if (!isError) {
            ...
            }, 5000); // Hide after 5 seconds for success messages
        }
    }
...
function updateStripeOptions(orientation, previousOrientation) {
    for (let tabNumber = 1; tabNumber <= 4; tabNumber++) {
        const select = document.getElementById(`leg-stripes-select-tab${tabNumber}`);
        ...
        if (previousOrientation === 'horizontal' && orientation === 'vertical') {
...
document.querySelectorAll('input[name="orientation1"]').forEach(radio => {
    radio.addEventListener('change', (e) => {
        ...
        const previousOrientation = document.querySelector('input[name="orientation1"]:not(:checked)').value;
        updateStripeOptions(newOrientation, previousOrientation);
    });
});

// Initial setup based on default selected orientation
const initialOrientation = document.querySelector('input[name="orientation1"]:checked').value;
updateStripeOptions(initialOrientation);
