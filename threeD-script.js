...
/* import deleteIconSVG from './icons/delete.svg';
...
let lastElasticRibLegColor = null;
let lastLegStripeColorTab1 = null;
let lastLegStripeColorTab2 = null;
let lastLegStripeColorTab3 = null;
let lastLegStripeColorTab4 = null;
let lastCuffStripeColorTab1 = null;
...
const updateAllInitial3DSettingsForSock = (config) => {
    ...
    updateStripesOrientationUI(legStripesOrientation, "orientation1");
    ...
    cuffStripesOrientation = config["leg"]["stripes"]["orientation"];
    ...
    updateStripesOrientationUI(cuffStripesOrientation, "orientation2");
...
const updateLegColor = (color) => {
    ...
    updateElasticRibLegColor(lastElasticRibLegColor);
}
...
const applyImageToLeg = () => {
    ...
    scene.traverse((child) => {
        if (child instanceof THREE.Mesh) {
            ...
            if (child.material.name.includes('leg')) {
...
legStripesOrientationSelect.addEventListener('change', (event) => {
    if (event.target.type === 'radio') {
        ...
        updateStripesOrientationUI(orientation, "orientation1");
...
const updateStripesOrientationUI = (orientation, name) => {
    document.querySelectorAll('.radio-options').forEach(group => {
        const background = group.querySelector('.radio-background');
        
        // Remove existing listeners first
        const radios = group.querySelectorAll(`input[type="radio"][name="${name}"]`);
        radios.forEach(radio => {
            radio.removeEventListener('change', radio.orientationChangeHandler);
        });

        // Add new listeners
        radios.forEach(radio => {
            // Store the handler function so we can remove it later
            radio.orientationChangeHandler = () => {
            ...
            radio.addEventListener('change', radio.orientationChangeHandler);
        });
...
cuffStripesOrientationSelect.addEventListener('change', (event) => {
    if (event.target.type === 'radio') {
        ...
        updateStripesOrientationUI(orientation, "orientation2");
...
// let cuffOpacityTexture = textureLoader.load('textures/socks_cuff_transparency.png');
...
const euler = new THREE.Euler(Math.PI * 2 / 180, 0, 0); // Rotate around X axis
...
// const planeHelper = new THREE.PlaneHelper(clippingPlane, 2, 0x00ff00); // Size 2, green color
    ...
    if (intersects.length > 0) {
        ...
        if (activeObject) {
            ...
            if (activeObject.controls.deleteControl) {
                ...
                console.log("HitPoint: ", clickPositionCanvas);
                console.log("Delete Pos: ", deleteIconLeft, deleteIconTop, iconSize);
